  ...
  (lambda (rule)
    (let* ((clauses (tell rule 'get-english-transcription))
           ...
           (center-coord (tell *workspace-window* 'get-rule-coord
                          ...
                          total-height))
           (x1-border (- (first center-coord) (* 1/2 max-clause-width) x-extra))
           (x2-border (+ (first center-coord) (* 1/2 max-clause-width) x-extra))
           (y1-border (- (second center-coord) (* 1/2 height-minus-border)))
           (y2-border (+ (second center-coord) (* 1/2 height-minus-border)))
           ...
           (text-x0 (- (first center-coord) (* 1/2 max-clause-width)))
           (text-y0 (+ (- (second center-coord) (* 1/2 height-minus-border)) y-extra))
           (clause-pexps
             (map (lambda (i clause)
