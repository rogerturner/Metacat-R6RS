  ...
  (lambda (layout . optional-args)
    (let* ((top-window
            (if (null? optional-args)
              (make-bridge-themes-window (first layout))
              (make-bridge-themes-window (first layout) (first optional-args))))
           (bottom-window
             (if (null? optional-args)
               (make-bridge-themes-window (second layout))
               (make-bridge-themes-window (second layout) (second optional-args))))
           (vertical-window
             (if (null? optional-args)
               (make-bridge-themes-window (third layout))
               (make-bridge-themes-window (third layout) (third optional-args))))
  ...
  (lambda (layout . optional-args)
    (let* ((pixels
            (if (null? optional-args)
              ...
              (first optional-args)))
           (x-pixels (first pixels))
           (y-pixels (second pixels))
           (theme-type (first layout))
           ...
           (panels
             (map (lambda (i dimension relations)
                   (let* ((lower-left (compute-lower-left i))
                          ...
                          (lower-right `(,(first upper-right) ,(second lower-left)))
                          (upper-left `(,(first lower-left) ,(second upper-right)))
...
(define make-panel
  (lambda (graphics-window dimension theme-type panel-info dim-name
    ...
    (let* ((delta (* 2 (tell graphics-window 'get-width-per-pixel)))
           (ll `(,(+ (first lower-left) delta) ,(+ (second lower-left) delta)))
           (ul `(,(+ (first upper-left) delta) ,(- (second upper-left) delta)))
           (ur `(,(- (first upper-right) delta) ,(- (second upper-right) delta)))
           (lr `(,(- (first lower-right) delta) ,(+ (second lower-right) delta)))
         ...
         (record-case (rest msg)
           ...
           (set-theme-graphics-parameters (theme)
             (let* ((relation (tell theme 'get-relation))
                    ...
                    (center-coord (second info))
                    (text-coord (third info))
                    (normal-pexp
                      `(let-sgl ((font ,rel-normal-font)
                                 (text-justification center))
                        (text ,text-coord ,(fourth info))))
                    (highlight-pexp
